...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    private fun hasVisibleAudioOnlyCoverPixels(bitmap: Bitmap): Boolean {
        if (bitmap.width <= 1 || bitmap.height <= 1)
            return false

        // Reject only a nearly uniform black/transparent frame. This protects devices where a
        // screenshot requested during the first render callback still returns the cleared buffer.
        // The real mpv surface remains visible while another capture is attempted.
        val samplesPerAxis = 12
        var visibleSamples = 0
        var brightest = 0
        var darkest = 255
        for (iy in 0 until samplesPerAxis) {
            val y = ((bitmap.height - 1L) * iy / (samplesPerAxis - 1)).toInt()
            for (ix in 0 until samplesPerAxis) {
                val x = ((bitmap.width - 1L) * ix / (samplesPerAxis - 1)).toInt()
                val pixel = bitmap.getPixel(x, y)
                if (Color.alpha(pixel) <= 16)
                    continue
                visibleSamples += 1
                val level = maxOf(Color.red(pixel), Color.green(pixel), Color.blue(pixel))
                brightest = maxOf(brightest, level)
                darkest = minOf(darkest, level)
            }
        }
        return visibleSamples > 0 && (brightest > 12 || brightest - darkest > 8)
    }
    ...
    private fun captureAudioOnlyCoverSnapshot(): Boolean {
        ...
        // TextureView.getBitmap() can return a completely black buffer on some devices even
        // though mpv is already showing the embedded cover. Read the decoded frame directly
        // from mpv instead. This happens only once per file; later aspect/zoom changes affect
        // the detached ImageView and never touch mpv's surface or video properties.
        val cover = try {
            MPVLib.grabVideoFrame(AUDIO_ONLY_COVER_CAPTURE_MAX_EDGE)
        } catch (_: Throwable) {
            null
        } ?: return false
        if (!hasVisibleAudioOnlyCoverPixels(cover)) {
            try { cover.recycle() } catch (_: Throwable) {}
            return false
        }
...
private fun openAdvancedMenu(restoreState: StateRestoreCallback) {
    ...
    private fun updateAudioUI() {
        ...
        // Audio cover-art aspect changes are handled on a detached ImageView. This avoids both a
