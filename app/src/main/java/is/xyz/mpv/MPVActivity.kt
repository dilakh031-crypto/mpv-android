...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    private fun scheduleSingleTapToggle() {
        // Wait for the single-tap window to expire. If a double-tap gesture fires
        // within DOUBLE_TAP_WINDOW_MS, it will call cancelPendingTapToggle() and
        // we will never toggle the control UI (no flash).
        cancelPendingTapToggle()
        val r = Runnable {
            pendingTapToggle = null
            toggleControls()
        ...
        pendingTapToggle = r
        tapToggleHandler.postDelayed(r, SINGLE_TAP_TOGGLE_WINDOW_MS)
    }
...
private fun openAdvancedMenu(restoreState: StateRestoreCallback) {
    ...
    companion object {
        ...
        // Single-tap window (ms): only after this delay we toggle the control UI.
        // This prevents double-tap actions (225ms window) from flashing the UI.
        private const val SINGLE_TAP_TOGGLE_WINDOW_MS = 245L
