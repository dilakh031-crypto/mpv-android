 ...
 *  - Unzoomed playback always uses the TextureView's exact pixel size. This gives
 *    mpv one final screen-sized render target and avoids an extra Android scaling
 *    pass through a smaller media-aspect SurfaceTexture.
 *  - Zoom upgrades to a source-detail surface only when the complete allocation
 *    is reasonably safe. Oversized requests are never partially capped: they
 *    fall back directly to the exact screen-sized surface instead.
...
internal class VideoZoomGestures(
    ...
    private fun requestViewAspectOriginalRenderSurfaceSize(force: Boolean) {
        ...
        // Keep the surface view-shaped while panscan is active. Request the full
        // source-detail allocation only when it is safe; do not ask SurfaceTexture
        // for an arbitrary partially capped intermediate size.
        val bufferSize = originalDetailRenderSurfaceSizeOrNull(
        ...
        player.setRenderSurfaceSize(bufferSize.first, bufferSize.second)
    ...
    private fun requestMediaAspectOriginalRenderSurfaceSize(force: Boolean) {
        ...
        // The compact media-aspect surface is useful while zoomed because it does
        // not waste pixels on black bars. Use it only when the complete requested
        // size is safe. Otherwise keep the exact screen-sized base surface.
        val bufferSize = originalDetailRenderSurfaceSizeOrNull(
    ...
    private fun requestMediaAspectBaseRenderSurfaceSize(force: Boolean) {
        // At normal zoom, render directly to the TextureView's exact dimensions.
        // A smaller media-aspect buffer is first expanded by SurfaceTexture and
        // then reduced again by the View transform, which can soften fine detail.
        requestBaseRenderSurfaceSize(force)
    }
    ...
    private fun originalDetailRenderSurfaceSizeOrNull(
        baseWidth: Double,
        baseHeight: Double,
        content: ContentRect,
    ): Pair<Int, Int>? {
        val desiredScale = originalDetailBufferScale(content)
        val desiredWidth = baseWidth.coerceAtLeast(1.0) * desiredScale
        val desiredHeight = baseHeight.coerceAtLeast(1.0) * desiredScale

        // A SurfaceTexture can consume several copies of its RGBA buffer. The old
        // 8192x8192 allowance could therefore request hundreds of megabytes and
        // make some devices silently reduce the buffer, show black, or terminate
        // the process. Keep a modest source-detail budget relative to the display.
        val screenWidth = viewWidth.toDouble().coerceAtLeast(1.0)
        val screenHeight = viewHeight.toDouble().coerceAtLeast(1.0)
        val screenPixels = screenWidth * screenHeight
        val safePixelBudget = (screenPixels * MAX_DETAIL_PIXEL_MULTIPLIER)
            .coerceAtLeast(screenPixels)
            .coerceAtMost(MAX_RENDER_SURFACE_PIXELS)
        val safeEdge = max(MAX_RENDER_SURFACE_EDGE, max(screenWidth, screenHeight))

        if (desiredWidth > safeEdge || desiredHeight > safeEdge ||
            desiredWidth * desiredHeight > safePixelBudget
        ) return null

        return ceilToIntAtLeastOne(desiredWidth) to ceilToIntAtLeastOne(desiredHeight)
    }
    ...
    companion object {
        ...
        // 4096 is a conservative cross-device SurfaceTexture edge. The pixel
        // budget is additionally tied to the actual display below.
        private const val MAX_RENDER_SURFACE_EDGE = 4096.0
        private const val MAX_RENDER_SURFACE_PIXELS = 16_777_216.0 // 4096 * 4096
        private const val MAX_DETAIL_PIXEL_MULTIPLIER = 4.0
