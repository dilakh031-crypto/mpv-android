...
abstract class BaseMPVView(context: Context, attrs: AttributeSet) : TextureView(context, attrs), TextureView.SurfaceTextureListener {
    init {
        // TextureView provides frame-available callbacks needed by the startup
        // blackout. Its buffer is kept at the real view size; zooming is done by mpv.
        isOpaque = true
    }
    ...
    /** Keep mpv's output surface at the real on-screen view size. */
    fun resetRenderSurfaceSize() {
        updateRenderSurfaceSize(width, height, forceBufferUpdate = false)
    }
    ...
    private fun updateRenderSurfaceSize(
        ...
        if (!forceBufferUpdate &&
            safeWidth == renderSurfaceWidth && safeHeight == renderSurfaceHeight
        ) return

        renderSurfaceWidth = safeWidth
        renderSurfaceHeight = safeHeight
        attachedTexture?.setDefaultBufferSize(renderSurfaceWidth, renderSurfaceHeight)
        if (attachedSurface != null) {
    ...
    private fun attachSurfaceTexture(texture: SurfaceTexture, width: Int, height: Int) {
        ...
        updateRenderSurfaceSize(width, height, forceBufferUpdate = true)
    ...
    override fun onSurfaceTextureSizeChanged(surface: SurfaceTexture, width: Int, height: Int) {
        updateRenderSurfaceSize(width, height)
    }
