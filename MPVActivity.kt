...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    private fun syncZoomVideoGeometry(
        ...
        val size = try { player.getVideoPixelSize() } catch (_: Throwable) { null }
        val geometry = if (audioOnlyAspectRenderingActive) {
            audioOnlyAspectGeometry()
        } else {
    ...
    private fun audioOnlyAspectGeometry(): Pair<Double?, Double> {
        val nativeAspect = try { player.getVideoAspect() } catch (_: Throwable) { null }
    ...
    private fun beginAudioOnlyAspectRendering() {
        ...
        audioOnlySavedKeepAspect = try { MPVLib.getPropertyBoolean("keepaspect") } catch (_: Throwable) { null }
        audioOnlyAspectRenderingActive = true

        if (::zoomGestures.isInitialized) {
            zoomGestures.setAudioOnlyVisual(true)
            syncZoomVideoGeometry(prepareNormalSurface = true, immediate = true)
        }

        // mpv now always fills the stable SurfaceTexture. The selected aspect is applied only
        // as an Android view transform, so changing it never touches mpv's video output.
        try { MPVLib.setPropertyBoolean("keepaspect", false) } catch (_: Throwable) {}
    }
    ...
    private fun endAudioOnlyAspectRendering() {
        ...
        val restoreKeepAspect = audioOnlySavedKeepAspect
        audioOnlyAspectRenderingActive = false
        audioOnlyAspectSelection = null
        audioOnlySavedKeepAspect = null

        try { MPVLib.setPropertyBoolean("keepaspect", restoreKeepAspect ?: true) } catch (_: Throwable) {}
        if (::zoomGestures.isInitialized) {
            zoomGestures.setAudioOnlyVisual(false)
            syncZoomVideoGeometry(prepareNormalSurface = true, immediate = true)
        }
    }
    ...
    // For audio-only files, aspect changes are rendered entirely by Android's TextureView.
    // Sending video-aspect-override to mpv for an attached cover image can reconfigure the
    // still-image video output and briefly starve audio on some devices.
    private var audioOnlyAspectRenderingActive = false
    private var audioOnlyAspectSelection: String? = null
    private var audioOnlySavedKeepAspect: Boolean? = null
    ...
    fun openAspectRatioDialog() {
        ...
        val dialog = with(AlertDialog.Builder(this)) {
            setSingleChoiceItems(names, selectedIndex) { _, item ->
                ...
                if (audioOnlyAspectRenderingActive) {
                    // Do not send any aspect/panscan property to mpv for audio cover art.
                    // The predicted TextureView geometry above is the final geometry.
